...
class _MyHomePageState extends State<MyHomePage> {
  ...
  int totalItems = 0;
  Input$CollectionSortParameter sort = Input$CollectionSortParameter(createdAt: Enum$SortOrder.ASC);
  Input$CollectionFilterParameter filter = Input$CollectionFilterParameter(); //parentId: Input$IDOperators (eq:"1")
  ...
  Future<QueryResult<Query$GetCollections>> _getAllCategories(BuildContext context) async {
    isFetching = true;
    cursor = 0;
    take = 100;
    skip = cursor * take;
    input = Input$CollectionListOptions(skip: skip, take: take, sort: sort, filter: filter, topLevelOnly: topLevelOnly); //, filterOperator: filterOperator
    variables = Variables$Query$GetCollections(options: input);
    options = Options$Query$GetCollections( variables: variables); //, fetchPolicy: FetchPolicy.networkOnly
